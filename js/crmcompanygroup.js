...
//	initModal();
	...
	$('#addFun').on('click', function() {
	    location.href = 'crmcompanygroupadd.html?action=add';
	})
        ...
        //}],
		 "aoColumns": [
		 	...
		 	{ "mDataProp": "cg_addTime",
	    		"createdCell": function(td, cellData, rowData, row, col) {
	    		    if (rowData.cg_addTime != null) {
	    		        $(td).html(rowData.cg_addTime.substring(0, 10));
	    			} else {
	    				$(td).html("NULL");
	    			}
	    		}
...
function _deleteFun(id) {
	bootbox.confirm("Are you sure?", function(result) {
		if(result) {
			$.ajax({
			    url: dataUrl + 'ajax/crmcompanygroup.ashx?action=cancelcrmCompanyGroup',
				data: {
					"Id": id
				},
				dataType: "json",
